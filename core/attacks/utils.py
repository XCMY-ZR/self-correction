...
#The following code is written by Ray at 2023/09/10
def HNloss(output,n_output,target):
    """
    HNloss for finding adversarial example to avoid noise destroy
    """
    loss1 = F.cross_entropy(output,target)
    loss2 = torch.norm(F.softmax(n_output) - F.softmax(output),1)  # l1(noisy_img-origin_img), should descend
    loss = loss1+loss2
    return loss

# class HN_loss(nn.Module):
#     def __init__(self,model,x_input):
#         super(HN_loss,self).__init__()
#         self.model = model
...
def DLRloss_target(x,y,y_target):
    """
    https://arxiv.org/pdf/2003.01690.pdf

    Difference of Logits Ratio Loss for target attack
    """
    x_sorted, ind_sorted = x.sort(dim=1)
    u = torch.arange(x.shape[0])
