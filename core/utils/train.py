...
class Trainer(object):
    ...
    def __init__(self, info, args):
        ...
        if self.params.pretrained_file is not None:
            print('load model from {}'.format(self.params.pretrained_file))
            self.load_model(self.params.pretrained_file)
    ...
    def standard_loss(self, x, y):
        ...
        preds = out.detach()
        batch_metrics = {'loss': loss.item(), 'clean_acc': accuracy(y, preds),'adversarial_acc':0}
    ...
    def eval_save_statistic(self, dataloader, epoch, prefix='test'):
        """
        Evaluate performance of the model and save the probability(adv and clean) for each sample
        """
        acc , adv_acc = 0.0 , 0.0
        self.model.eval()
        LOG_DIR = os.path.join(self.params.log_dir, self.params.desc)
        metrics = pd.DataFrame()
        for x, y in dataloader:
            x, y = x.to(device), y.to(device)
            with ctx_noparamgrad_and_eval(self.model):
            ...
            acc += accuracy(y, out)
            adv_acc += accuracy(y, adv_out)

            pred = torch.max(out,dim=1)[1]
            prob = torch.max(out.softmax(dim=1),dim=1)[0]

            adv_pred = torch.max(adv_out,dim=1)[1]
            adv_prob = torch.max(adv_out.softmax(dim=1),dim=1)[0] 

            batch_metrics = {'true_label': y.cpu().numpy(), 'clean_pred': pred.cpu().numpy(),
                            'clean_prod': prob.cpu().numpy(), 'adv_pred': adv_pred.cpu().numpy(),
                            "adv_prob": adv_prob.cpu().numpy() }
            metrics = metrics.append(pd.DataFrame(batch_metrics),ignore_index=True)

        metrics.to_csv(os.path.join(LOG_DIR, '{}_statistic_{}.csv'.format(prefix,epoch)), index=False)
        acc /= len(dataloader)
        adv_acc /= len(dataloader)
        return acc , adv_acc
