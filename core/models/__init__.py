...
def create_model(name, normalize, info, device, use_FNandWN=False):
    """
    ...
    if info['data'] in ['tiny-imagenet']:
        ...
        if 'preact-resnet' in name and 'swish' not in name:
            ...
            if use_FNandWN:
                print('Use {} with FNandWN'.format(name))
                backbone = wideresnetwithswish(name, dataset=info['data'], num_classes=info['num_classes'], device=device,use_FNandWN=use_FNandWN)
            else:
