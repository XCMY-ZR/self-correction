...
class WideResNet(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_FNandWN:
            self.logits = nn.Linear(num_channels[3], num_classes,bias = False)
        else:
    ...
    def forward(self, x):
        ...
        if self.use_FNandWN:
            ...
            for _, module in self.logits.named_modules():
                if isinstance(module, nn.Linear):
                    module.weight.data = F.normalize(module.weight, p=2, dim=1)
        return self.logits(out)
    
    
def wideresnetwithswish(name, dataset='cifar10', num_classes=10, device='cpu',use_FNandWN=False):
    """
    ...
    if 'cifar100' in dataset:
        return WideResNet(num_classes=num_classes, depth=depth, width=widen, activation_fn=_ACTIVATION[act_fn], 
                          mean=CIFAR100_MEAN, std=CIFAR100_STD, use_FNandWN=use_FNandWN)
    elif 'svhn' in dataset:
        return WideResNet(num_classes=num_classes, depth=depth, width=widen, activation_fn=_ACTIVATION[act_fn], 
                          mean=SVHN_MEAN, std=SVHN_STD, use_FNandWN=use_FNandWN)
    return WideResNet(num_classes=num_classes, depth=depth, width=widen, activation_fn=_ACTIVATION[act_fn],use_FNandWN=use_FNandWN)